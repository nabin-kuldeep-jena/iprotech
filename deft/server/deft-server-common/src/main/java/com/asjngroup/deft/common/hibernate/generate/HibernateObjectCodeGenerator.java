...
public class HibernateObjectCodeGenerator
{
				...
				else
				{
					...
					for ( DatabaseSchema modelSchema : metadata.getDatabaseRootSchema().getDatabaseSchemas() )
					{
						...
						else
						{
							...
							if ( result == 0 )
								throw new DeftRuntimeException( "Two schemas with identical versions '%1'", schema.toString() );
					...
					if ( currentLatestSchema == null )
						throw new DeftRuntimeException( "No schemas found in package '%1'", metadata.getName() );
	...
	private void processFile( List<SchemaTag> paramList, List<String> paramList1, SchemaTag paramSchemaTag, File paramFile, ArrayList<String> paramArrayList ) throws Exception
	{
		...
		if ( ( !( localFile1.exists() ) ) && ( !( localFile1.mkdirs() ) ) )
			throw new DeftRuntimeException( "Failed to create directory '%1'", new Object[]
			{ localFile1 } );
	...
	protected void checkDirectory( File paramFile ) throws DeftRuntimeException
	{
		if ( !( paramFile.exists() ) )
			throw new DeftRuntimeException( "Directory '%1' does not exist", new Object[]
			{ paramFile.getAbsolutePath() } );
		if ( paramFile.isDirectory() )
			return;
		throw new DeftRuntimeException( "Directory '%1' is not a directory", new Object[]
		{ paramFile.getAbsolutePath() } );
	}
