...
import com.asjngroup.deft.common.database.hibernate.references.TableDfn;
import com.asjngroup.deft.common.database.hibernate.references.TableIndex;
import com.asjngroup.deft.common.database.hibernate.references.TableIndexColumn;
import com.asjngroup.deft.common.database.hibernate.references.TableInst;
...
public class SchemaHelper
{
	...
	public static Schema buildSchema( String serverSchema, String mappingSchemaPath ) throws SchemaParseException
	{
		InputStream inputStream = SchemaHelper.class.getResourceAsStream( serverSchema );

		if ( inputStream == null )
		{
			throw new SchemaParseException( "Could not find schema file %1", serverSchema );
		}
		Schema schema = Schema.loadSchema( inputStream, true );

		inputStream = SchemaHelper.class.getResourceAsStream( mappingSchemaPath );

		if ( inputStream == null )
		{
			throw new SchemaParseException( "Could not find schema file %1", mappingSchemaPath );
		}

		Schema mappingSchema = Schema.loadSchema( inputStream, false );

		mergeClientAndServerSchema( schema, mappingSchema );
		
		return mappingSchema;
	}

	public static void mergeClientAndServerSchema( Schema serverSchema, Schema mappingSchema )
	{
		mappingSchema.Databases = serverSchema.Databases;
	}

	public static Schema buildSchema( String schemaPath, boolean isCamelCase ) throws SchemaParseException
	{
		...
		return Schema.loadSchema( inputStream, isCamelCase );
	}

	public static Schema buildSchema( List<String> schemaPaths, boolean isCamelCase ) throws SchemaParseException
	{
		...
		for ( String schemaPath : schemaPaths )
		{
			if ( schema == null )
			{
				schema = buildSchema( schemaPath, isCamelCase );
			}
			else
			{
				schema.mergeSchema( buildSchema( schemaPath, isCamelCase ) );
			}
		}
	...
	public static Map<String, Map<String, List<String>>> buildUniqueIndexMap( Schema schema )
	{
		...
		for ( Database database : schema.Databases )
		{
			...
			{
				Entity entityMapping = schema.findEntityFromTableName( database.DatabaseName, table.TableName );
				...
				for ( Index index : table.Indexes )
				{
					...
					{
						Field fieldMapping = entityMapping.findFieldMappingByColumn( indexColumn );
	...
	public static void saveToV5SchemaFile( Schema schema, String filename ) throws IOException
	{
		...
		writer.println( "  <Schema Name=\"" + schema.schemaName + "\" majorVersion=\"" + schema.majorVersion + "\" minorVersion=\"" + schema.minorVersion + "\" revisionVersion=\"" + schema.revisionVersion + "\" ServicePack=\"" + schema.servicePack + "\" IsExtension=\"0\">" );
		...
		for ( Database database : schema.Databases )
		{
			...
			for ( Table table : database.Tables )
			{
				...
				for ( Column column : table.Columns )
				{
					...
					writer.println( "              <Column Column=\"" + column.ColumnName + "\" Datatype=\"" + datatype + "\" Mandatory=\"" + ( column.IsMandatory ? "-1" : "0" ) + "\" IsPrimaryKey=\"" + ( column.IsPrimaryKey ? "-1" : "0" ) + "\" ExtensionKey=\"SCHEMA\" />" );
				}
				writer.println( "            </Columns>" );
				// Ignoring indexes...
	...
	public static String getPrimaryKeyColumnName( String tableName )
	{
		...
		if ( tableInsts.size() == 0 )
			throw new DeftRuntimeException( "Table '%1' is not a valid table instance", tableName );
		...
		if ( tableColumns.size() == 0 )
			throw new DeftRuntimeException( "Table '%1' does not have a primary key column", tableName );
	...
	public static ColumnDataType[] getColumnDataTypesForColumns( String tableName, String[] columnNames ) throws HibernateException
	{
		...
		for ( String columnName : columnNames )
		{
			...
			if ( !foundColumn )
				throw new DeftRuntimeException( "Column '%1' is not a valid column name for table '%2'", columnName, tableName );
		}
	...
	{
		ColumnDataType[] ColumnDataTypes = getColumnDataTypesForColumns( tableName, new String[]
		{ columnName } );
	...
	public static void validateTable( Table table )
	{
		if ( table.TableName.length() > 4 && table.TableName.substring( table.TableName.length() - 4, table.TableName.length() ).equals( "_adt" ) )
		{
			if ( table.TableName.length() > MAX_AUDITING_TABLE_NAME_LENGTH )
				throw new DeftRuntimeException( "Table name '%1' is too long (max %2 chars supported)", table.TableName, MAX_AUDITING_TABLE_NAME_LENGTH );
		}
		else
		{
			if ( table.TableName.length() > MAX_TABLE_NAME_LENGTH )
				throw new DeftRuntimeException( "Table name '%1' is too long (max %2 chars supported)", table.TableName, MAX_TABLE_NAME_LENGTH );
		}
	...
	public static void validateIndex( Index index )
	{
		if ( index.IndexName.length() > MAX_INDEX_NAME_LENGTH )
			throw new DeftRuntimeException( "Index name '%1' is too long (max %2 chars supported)", index.IndexName, MAX_INDEX_NAME_LENGTH );
	}

	public static void validateColumn( Column column )
	{
		if ( column.ColumnName.length() > MAX_COLUMN_NAME_LENGTH )
			throw new DeftRuntimeException( "Column name '%1' is too long (max %2 chars supported)", column.ColumnName, MAX_COLUMN_NAME_LENGTH );
	}
	...
	public static int getUsageGroupId( String tableName )
	{
		try
		{
			...
			if ( usgId == null )
				throw new DeftRuntimeException( "Table '%1' is not a partitioned table", tableName );

			return usgId;
		}
		catch ( HibernateException e )
		{
			throw new DeftRuntimeException( "Failed to find the Table Instance with TinTableName = '%1'", tableName );
		}
	}
