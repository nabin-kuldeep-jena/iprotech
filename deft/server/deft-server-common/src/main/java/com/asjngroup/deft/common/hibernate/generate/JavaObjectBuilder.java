...
import com.asjngroup.deft.common.database.hibernate.ColumnDataType;
...
public class JavaObjectBuilder
{
	...
	public void generateImplementation( PrintWriter paramPrintWriter, ArrayList<String> paramArrayList ) throws GenerateException
	{
		...
		outputFilterDfns( paramPrintWriter );
	...
	private void outputFilterDfns( PrintWriter writer )
	{
		List<String> filterStrings = new ArrayList<String>();

		if ( !entityMapping.getEntityName().equals( "PartitionTbl" ) )
		{
			filterStrings.add( "@org.hibernate.annotations.FilterDef( name = \"partitionFilter\", parameters = @org.hibernate.annotations.ParamDef(name = \"partitionIds\", type = \"int\"))" );
		}

		filterStrings.add( "@org.hibernate.annotations.FilterDef( name = \"deletedFilter\", parameters = @org.hibernate.annotations.ParamDef(name = \"deleteFl\", type = \"yes_no\"))" );

		writer.println( "@org.hibernate.annotations.FilterDefs( {" );

		int size = filterStrings.size();
		for ( int i = 0; i < size; i++ )
		{
			writer.print( indent( 1 ) + filterStrings.get( i ) );
	...
	private void outputImports( PrintWriter paramPrintWriter, boolean paramBoolean, ArrayList<String> paramArrayList )
	{
		...
		paramPrintWriter.println();
		paramPrintWriter.println( "import com.asjngroup.deft.common.database.hibernate.references.*;" );
		paramPrintWriter.println( "import com.asjngroup.deft.common.util.StringUtil;" );
		paramPrintWriter.println( "import com.asjngroup.deft.common.util.ResourceManager;" );
		...
		if ( paramBoolean )
		{
			...
			paramPrintWriter.println( "import com.asjngroup.deft.common.database.helper.IdHelper;" );
			//paramPrintWriter.println( "import com.asjngroup.deft.database.hibernate.audit.AuditHelper;" );
			//paramPrintWriter.println( "import com.asjngroup.deft.database.hibernate.mappings.reference.*;" );
			//paramPrintWriter.println( "import com.asjngroup.deft.database.hibernate.util.*;" );
			if ( !( "com.asjngroup.deft.common.database.hibernate.references".equals( this.schema.getServerPackage() ) ) )
	...
	{
		paramPrintWriter.print( "@com.asjngroup.deft.common.hibernate.annotation.Table( " );
	...
	private void outputFilters( PrintWriter paramPrintWriter )
	{
		...
		if ( !entityMapping.getEntityName().equals( "PartitionTbl" ) )
		{
			filterStrings.add( "@org.hibernate.annotations.Filter( name = \"partitionFilter\", condition = \"ptn_id in ( :partitionIds )\" )" );
		}

		filterStrings.add( "@org.hibernate.annotations.Filter( name = \"deletedFilter\", condition = \"delete_fl = :deleteFl\" )" );
		...
		for ( int i = 0; i < size; i++ )
		{
			paramPrintWriter.print( indent( 1 ) + filterStrings.get( i ) );
	...
	public static String getHibernateType( ColumnDataType paramColumnDataType ) throws GenerateException
	{
		...
		case 4:
			return "yes_no";
		case 5:
			return "com.asjngroup.deft.common.database.hibernate.type.DecimalType";
		case 6:
			return "text";
		case 7:
