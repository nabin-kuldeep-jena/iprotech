...
public class MySqlDataSource extends AbstractDataSource
{
	...
	public void decorateResultSet( ResultSet rs )
	{
		...
		{
			throw new DeftRuntimeException( e.getCause() );
		}
	...
	public String getDropIndexDDL( String tableName, String indexName, boolean checkExists ) throws DataSourceException
	{
		try
		{
			...
			{
				// Check for existence
				statement = "select 1" + StringHelper.NEW_LINE + "            from information_schema.statistics " + StringHelper.NEW_LINE + "            where table_name    = '" + tableName + "'" + StringHelper.NEW_LINE + "            and index_name    = '" + indexName + "'" + StringHelper.NEW_LINE+" AND TABLE_SCHEMA='"+getDbDatabaseName()+"'";
				if ( doesItExist( statement ) )
				{
					statement = "drop index " + indexName + " on " + tableName + StringHelper.NEW_LINE;
				}
				// + "    drop index " + index.table.TableName + "." + index.IndexName + StringHelper.NEW_LINE;
			}
			else
			{
				// Just drop the index
	...
	private String getCreateIndexStatement( Index index, String indexLocation ) throws DataSourceException
	{
		try
		{
			...
			// Check index location is not specified
			if ( indexLocation != null && indexLocation.length() > 0 )
				throw new DataSourceException( "Specififying index locations is not supported for MYSQL" );

			// create the index if it does not already exist
			statement = "select 1" + StringHelper.NEW_LINE + "            from information_schema.statistics " + StringHelper.NEW_LINE + "            where table_name    = '" + index.table.TableName + "'" + StringHelper.NEW_LINE + "            and index_name    = '" + index.IndexName + "'" + StringHelper.NEW_LINE+ " AND TABLE_SCHEMA='"+getDbDatabaseName()+"'";
			// + "create" + ( index.IsUnique ? " unique" : "" ) + " " + ( index.IsClustered ? "clustered" : "nonclustered" ) + " index " + index.IndexName + StringHelper.NEW_LINE
			// + "on " + index.table.TableName + "(" + index.ColumnList + ")" + StringHelper.NEW_LINE;

			if ( !doesItExist( statement ) )
			{
				statement = "create" + ( index.IsUnique ? " unique" : "" ) + " index " + index.IndexName + StringHelper.NEW_LINE + "on " + index.table.TableName + "(" + index.ColumnList + ")" + StringHelper.NEW_LINE;
	...
	public String getRenameTableDDL( String fromTable, String toTable, boolean checkExists ) throws DataSourceException
	{
		try
		{
			...
			if ( checkExists && tableExists( fromTable ) )
			{
	...
	public String getCreateIndexDDL( String tinTableName, String indexName, boolean uniqueFl, String columnList, String indexLocation ) throws DataSourceException
	{
		...
		// Check index location is not specified
		if ( indexLocation != null && indexLocation.length() > 0 )
			throw new DataSourceException( "Specififying index locations is not supported for SQL Server" );

		// create the index if it does not already exist
		statement = "select 1" + StringHelper.NEW_LINE + "            from information_schema.statistics " + StringHelper.NEW_LINE + "            where table_name    = '" + tinTableName + "'" + StringHelper.NEW_LINE + "            and index_name    = '" + indexName + "'" + StringHelper.NEW_LINE;
		// + "create" + ( index.IsUnique ? " unique" : "" ) + " " + ( index.IsClustered ? "clustered" : "nonclustered" ) + " index " + index.IndexName + StringHelper.NEW_LINE
		// + "on " + index.table.TableName + "(" + index.ColumnList + ")" + StringHelper.NEW_LINE;

		if ( !doesItExist( statement ) )
		{
			statement = "create" + ( uniqueFl ? " unique" : "" ) + " index " + indexName + StringHelper.NEW_LINE + "on " + tinTableName + "(" + columnList + ")" + StringHelper.NEW_LINE;
	...
	protected String getTableQuery( String database, String tableName, boolean exact )
	{
		...
		sb.append( " where     tab.OWNER             = '" + getDbDatabaseName().toUpperCase() + "' " );
	...
	protected ColumnDataType dbTypeToColumnDataType( String dbType, int length, Integer scale, boolean syncWithNonNcash ) throws DataSourceException
	{
		...
		if ( ( dbType.equalsIgnoreCase( "number" ) || dbType.equalsIgnoreCase( "int" ) ) && length <= 10 )
			return ColumnDataType.Int;
		if ( ( dbType.equalsIgnoreCase( "number" ) || dbType.equalsIgnoreCase( "bigint" ) ) && ( length <= 19 || ( length >= 20 && syncWithNonNcash ) ) )
			return ColumnDataType.Long;
		if ( dbType.equalsIgnoreCase( "nvarchar2" ) )
			return ColumnDataType.String;
	...
	public String getAllIndexDetailsQuery( String database, String tableName, boolean isViewRequired )
	{
		...
		sb.append( "where   inx.INDEX_SCHEMA           = '" + getDbDatabaseName() + "'        " );
		sb.append( "and   inx.TABLE_SCHEMA           = '" + getDbDatabaseName() + "'        " );
	...
	public String getAllTableDetailsQuery( String database, String tableName )
	{
		...
		sb.append( " where   tab.TABLE_NAME        = col.TABLE_NAME " );
		sb.append( " and     tab.TABLE_SCHEMA             = col.TABLE_SCHEMA " );
		sb.append( " and     tab.TABLE_SCHEMA             = '" + getDbDatabaseName() + "' " );
	...
	public List<String> getDropIndexDDL( Index index, boolean checkExists ) throws DataSourceException
	{
		...
		if ( checkExists )
		{
			statement = "select 1" + StringHelper.NEW_LINE + "            from information_schema.statistics " + StringHelper.NEW_LINE + "            where table_name    = '" + index.table.TableName + "'" + StringHelper.NEW_LINE + "            and index_name    = '" + index.IndexName + "'" + StringHelper.NEW_LINE+ " AND TABLE_SCHEMA='"+getDbDatabaseName()+"'";
			if ( !doesItExist( statement ) )
				statement = "ALTER TABLE " + index.table.TableName + " DROP INDEX " + index.IndexName;
			else
				statement = null;
			// Check for existence
